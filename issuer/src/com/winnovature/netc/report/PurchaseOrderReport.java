...
@WebServlet("/report/purchaseorder")
public class PurchaseOrderReport extends HttpServlet {
	private static final long serialVersionUID = 1L;

	static Logger log = Logger.getLogger(PurchaseOrderReport.class.getName());
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			finalResponse = new NETCReportsDAO().getPURCHASEORDERReport(fromDate, toDate, userId, conn);

		} catch (Exception e) {
			log.error(e);
			log.info(e.getMessage());
		} finally {
			log.info("*****************Response to /report/purchaseorder API()****************");
