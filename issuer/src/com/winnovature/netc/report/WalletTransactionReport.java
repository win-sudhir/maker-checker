...
@WebServlet("/report/wallettxn")
public class WalletTransactionReport extends HttpServlet {
	private static final long serialVersionUID = 1L;
	static Logger log = Logger.getLogger(WalletTransactionReport.class.getName());
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (!userId.startsWith("C")) {
				finalResponse = new NETCReportsDAO().getWalletTxnReport(fromDate, toDate, userId, conn);
			} else if (userId.startsWith("C")) {
				finalResponse = new NETCReportsDAO().getCustomerWalletTxnReport(fromDate, toDate, userId, conn);
			}
		...
		Exception e) {
			...
			log.info("*****************Response to /report/wallettxn API()****************");
