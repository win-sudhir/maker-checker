...
@WebServlet("/report/vehicle")
public class VehicleReport extends HttpServlet {
	private static final long serialVersionUID = 1L;
	static Logger log = Logger.getLogger(VehicleReport.class.getName());
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (!userId.startsWith("C")) {
				finalResponse = new NETCReportsDAO().getVEHICLEReport(fromDate, toDate, userId, conn);
			} else if (userId.startsWith("C")) {
				finalResponse = new NETCReportsDAO().getCustomerVEHICLEReport(fromDate, toDate, userId, conn);
			}
			...
			log.info("*****************Response to /report/vehicle API()****************");
