...
{
	static Logger log = Logger.getLogger(OrderMgmtDAO.class.getName());
	
	public JSONArray getExcelData(String poid)
	{
	...
	public List<List<String>> downloadExcel(String po_id)
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				/*data = data.put("TID", tid);
				data = data.put("TagID", tag_id);
				data = data.put("SignatureData", sagnature_data);
				data = data.put("BarcodeData", barcode_data);
				data = data.put("TagPassword", tag_pwd);
				data = data.put("KillPassword", kill_pwd);
				data = data.put("Date", date);
				tagdata.put(data);*/
				//data = tagdata.(data);  
	 			//log.info("Tag data......"+tagdata);
